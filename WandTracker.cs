...
namespace WiiWandz
{
    class WandTracker
    {
        ...
        public void initializeSpells()
        {
            if (device != null && authorization != null && spellNames != null)
            {
                ...
                for (int i = 0; i < spellNames.Count; i++)
                {
                    ...
                    object[] parms = new object[] { device, authorization, iftttEvent };
        ...
        public Spell addPosition(WiimoteInfo.Point pointF, DateTime dateTime)
        {
                    ...
                    if (wandIsPaused(positions))
                    {
                        ...
                        if (chosen != null && authorization != null && device != null && spellNames != null)
                        {
                            for (int i = 0; i < spellNames.Count; i++)
                            {
                                ...
                                {
                                    spell = chosen;
                                    ((CloudBitSpell)spell).setConfigurations(device, authorization, iftttUserKey, spellIftttEvents[i]);
                                    spell.castSpell();
